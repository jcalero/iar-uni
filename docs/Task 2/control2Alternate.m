function positions = control2Alternate(s,speed)
  ...
  while (iteration < 1000)
      ...
      xPos = 
        xPos + 
        0.5*(wheelDeltaL + wheelDeltaR)*0.08 * cos(angle);
      yPos = 
        yPos + 
        0.5*(wheelDeltaL + wheelDeltaR)*0.08 * sin(angle);

      positions = cat(1,positions,[xPos,yPos]);
      
      if((rightIR1 > 250 || rightIR2 > 400) 
            && (leftIR1 > 250 || leftIR2 > 400)
            && ((midRightIR + midLeftIR)/2 < 150))
      ...
      if(a > 0)
          rSpeed = speed;
          lSpeed = 
            max(round(speed - (12 * abs(a) / 180) -1),-speed);
      else
          lSpeed = speed;
          rSpeed = 
            max(round(speed - (12 * abs(a) / 180) -1),-speed);
